...
export class Glasium {
    private static shape(background: HTMLElement, {
        ...
        for (let i: number = 0; i < count; ++i) {
            /** insert shape */
            const shapeType: Glasium.Shape = shape === 'all'
            ...
            /** css value */
            let randomScale: number = library.randomBetween(0.4, 2, false) * scale
            let randomBrightness: number = library.randomBetween(
                color.brightness[0], color.brightness[1], false,
                [0.97, 1.03]
            )
            let size: number = 26 * randomScale
            let position: number = library.randomBetween(0, 100, false)
            let speedPerFiveSeconds: number = library.randomBetween(0.67, 1.35, false) * 5 / speed
            let delay: number = library.randomBetween(-speedPerFiveSeconds / 2, speedPerFiveSeconds / 2, false)
    ...
    static init(element: HTMLElement, {
        ...
        if (onMutation != undefined) {
            ...
            Glasium.init(element, {
                shape, color, scale, speed, count, rotate,
