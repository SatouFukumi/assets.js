...
export const tooltip: Tooltip.Interface = {
    ...
    addHook(on: Tooltip.Hook['on'], key: Tooltip.Hook['key'], {
        handler = ({ value }: { value: undefined | string }): string | undefined => value,
        follower = (): void => { },
        priority = 1,
        fit = false
    }: Tooltip.AddHook = {}): void {
    ...
    hide(): void {
        ...
        this.hideId = window.setTimeout((): void => {
            ...
            this.deactivateId = window.setTimeout((): void => {
    ...
    move: new RecordAnimationFrame(throttled(
        function (): void { tooltip.__move() },
        THROTTLE
    )),
